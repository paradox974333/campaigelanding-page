...
const BUSINESS_TYPES = [
    { value: '', label: 'Select type...' }, { value: 'cafe', label: 'Cafe' }, { value: 'restaurant', label: 'Restaurant' }, { value: 'hotel', label: 'Hotel' },
    { value: 'bar', label: 'Bar/Pub' }, { value: 'caterer', label: 'Caterer' }, { value: 'distributor', label: 'Distributor/Wholesaler' },
    { value: 'retailer', label: 'Retailer' }, { value: 'event_organizer', label: 'Event Organizer' }, { value: 'other', label: 'Other' },
] as const;
...
const HeroSection: React.FC<{ onCTAClick: () => void }> = ({ onCTAClick }) => (
  <section className="relative pt-20 pb-20 min-h-screen flex items-center justify-center gradient-hero overflow-hidden">
    ...
    <div className="relative text-center px-4 max-w-4xl mx-auto z-10">
      ...
      <h1 className="text-4xl sm:text-5xl md:text-7xl font-bold text-gray-900 mb-6 leading-tight animate-fadeInUp">{siteInfo.slogan}</h1>
      <p className="text-lg sm:text-xl md:text-2xl text-gray-700 mb-12 leading-relaxed max-w-3xl mx-auto animate-fadeInUp font-light" style={{animationDelay: '0.2s'}}>{siteInfo.brandIntro}</p>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-12 max-w-4xl mx-auto">
        {siteInfo.environmentalBenefits.map((benefit, index) => (
          <div key={benefit.id} className={`flex flex-col items-center p-4 bg-white/70 backdrop-blur-sm rounded-2xl border border-white/50 hover:bg-white/90 transition-all duration-300 hover:-translate-y-1 animate-fadeInUp`} style={{animationDelay: `${0.4 + index * 0.1}s`}}>
            <div className={`w-12 h-12 bg-${benefit.color}-100 rounded-xl flex items-center justify-center mb-3`}><benefit.icon className={`h-6 w-6 text-${benefit.color}-600`} /></div>
      ...
      <p className="text-sm text-gray-500 mt-4 animate-fadeInUp" style={{animationDelay: '1s'}}>No payment required • Free nationwide shipping • 48-hour dispatch</p>
    </div>
  </section>
);

interface ProductCardProps { product: ProductVariant; idx: number; onCTAClick: () => void; }
const ProductCard: React.FC<ProductCardProps> = React.memo(({ product, idx, onCTAClick }) => (
  <div className={`group bg-white rounded-3xl overflow-hidden shadow-lg hover:shadow-2xl border border-gray-100 transition-all duration-500 transform hover:-translate-y-2 animate-fadeInUp`} style={{ animationDelay: `${idx * 150}ms` }}>
    ...
    <div className="p-6 text-center">
      ...
      <button onClick={onCTAClick} className="w-full bg-emerald-50 hover:bg-emerald-100 text-emerald-700 font-semibold px-6 py-3 rounded-xl transition-all duration-300 hover:shadow-md border border-emerald-200 hover:border-emerald-300">
        Get FREE Sample
      </button>
    </div>
  </div>
  ...
  <section id="products" className="py-24 gradient-section">
    <div className="max-w-7xl mx-auto px-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 mb-16">
        {siteInfo.productVariants.map((product, idx) => (<ProductCard key={product.id} product={product} idx={idx} onCTAClick={onCTAClick} />))}
      </div>
      <div className="max-w-4xl mx-auto">
        <div className="bg-gradient-to-br from-blue-50 to-indigo-50 p-8 rounded-3xl border border-blue-100 shadow-lg">
          <h3 className="font-bold text-xl text-blue-800 mb-4 flex items-center gap-2"><Shield className="h-5 w-5" />The Superior Eco-Choice</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {siteInfo.environmentalBenefits.map((benefit, index) => (
          <div key={benefit.id} className={`flex items-start gap-6 p-8 bg-gray-50 rounded-3xl hover:bg-gray-100 transition-all duration-300 animate-slideInFromLeft`} style={{animationDelay: `${index * 0.2}s`}}>
            <div className={`w-16 h-16 bg-${benefit.color}-100 rounded-2xl flex items-center justify-center flex-shrink-0`}><benefit.icon className={`h-8 w-8 text-${benefit.color}-600`} /></div>
            <div>
    ...
    <section id="samples" className="py-24 gradient-section">
      <div className="max-w-lg mx-auto px-4">
        <div className="text-center mb-8">
          <div className="w-20 h-20 bg-emerald-100 rounded-3xl flex items-center justify-center mx-auto mb-6 animate-float"><Gift className="h-10 w-10 text-emerald-600" /></div>
        ...
        <form onSubmit={onSubmit} className="bg-white p-8 rounded-3xl shadow-xl border border-gray-100 space-y-6">
          <div className="text-center p-4 bg-gradient-to-r from-emerald-50 to-green-50 rounded-2xl border border-emerald-200">
            <p className="text-emerald-700 font-semibold text-sm flex items-center justify-center gap-2"><CheckCircle className="h-4 w-4" />Free campaign active - No payment required</p>
          </div>
          {FORM_FIELDS_CONFIG.map(field => (
            <div key={field.name}>
              ...
              {field.type === 'textarea' ? (
                <textarea name={field.name} id={field.name} value={formData[field.name as 'address']} onChange={onInputChange} required rows={3} placeholder={field.placeholder} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all duration-200 resize-none text-sm placeholder-gray-400 bg-gray-50 focus:bg-white" disabled={isSubmitting} autoComplete={field.autocomplete}/>
              ) : field.type === 'select' ? (
                <select name={field.name} id={field.name} value={formData[field.name as 'businessType']} onChange={onInputChange} required className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all duration-200 text-sm placeholder-gray-400 bg-gray-50 focus:bg-white appearance-none" disabled={isSubmitting} autoComplete={field.autocomplete}>
                  {field.options?.map(option => (<option key={option.value} value={option.value} disabled={option.value === ''}>{option.label}</option>))}
                </select>
                ...
                <div className="flex flex-wrap gap-2 p-3 border border-gray-200 rounded-xl bg-gray-50 focus-within:ring-2 focus-within:ring-emerald-500 focus-within:border-emerald-500">
                  {field.options?.map(option => (
                    <button type="button" key={option.value} onClick={() => handleMultiSelectChange(option.value)} disabled={isSubmitting}
                      className={`px-3 py-1.5 text-xs font-medium rounded-full border transition-all duration-200 ${formData.strawSizes.includes(option.value) ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-100 hover:border-gray-400'} ${isSubmitting ? 'cursor-not-allowed opacity-70' : 'cursor-pointer'}`}>
                      {option.label}
                    </button>
                  ))}
                </div>
              ) : (
                <input type={field.type as string} name={field.name} id={field.name} value={formData[field.name as Exclude<keyof FormData, 'address' | 'businessType' | 'message' | 'strawSizes'>]} onChange={onInputChange} required autoComplete={field.autocomplete} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all duration-200 text-sm placeholder-gray-400 bg-gray-50 focus:bg-white" placeholder={field.placeholder} disabled={isSubmitting}/>
              )}
            </div>
          ))}
          <div>
            <label htmlFor="message" className="block text-sm font-semibold text-gray-700 mb-2">Message (optional)</label>
            <textarea name="message" id="message" value={formData.message} onChange={onInputChange} rows={3} placeholder="Any specific requirements or questions?" className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all duration-200 resize-none text-sm placeholder-gray-400 bg-gray-50 focus:bg-white" disabled={isSubmitting}/>
          </div>
          <button type="submit" disabled={!isFormValid || isSubmitting}
            className={`w-full py-4 px-6 rounded-xl font-bold text-sm transition-all duration-300 flex items-center justify-center gap-2 ${isSubmitting ? 'bg-gray-400 text-gray-700 cursor-wait' : isFormValid ? 'bg-emerald-600 hover:bg-emerald-700 text-white shadow-lg hover:shadow-xl hover:shadow-emerald-500/25 hover:-translate-y-0.5' : 'bg-gray-200 text-gray-400 cursor-not-allowed'}`}>
            {isSubmitting ? (<><svg className="animate-spin -ml-1 mr-3 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>Submitting...</>) : (<><span>Send Sample Request</span><CheckCircle className="h-4 w-4" /></>)}
          </button>
  ...
  <footer className="bg-gray-900 text-gray-300 py-16">
    <div className="max-w-6xl mx-auto px-4">
      <div className="text-center mb-12">
        <div className="flex justify-center items-center mb-6"><img src={siteInfo.logoPath} alt={`${COMPANY_NAME} Logo`} className="h-12 w-12 object-contain mr-3"/><span className="text-2xl font-bold text-white">{COMPANY_NAME}</span></div>
        ...
        <div className="flex flex-col sm:flex-row justify-center items-center gap-6 text-sm">
          <a href={`mailto:${siteInfo.contactEmail}`} className="hover:text-emerald-400 transition-colors duration-200 flex items-center gap-2 group"><Mail className="h-4 w-4 group-hover:scale-110 transition-transform" />{siteInfo.contactEmail}</a>
          <a href={`https://wa.me/${TARGET_WHATSAPP_NUMBER.replace(/\D/g, '')}`} target="_blank" rel="noopener noreferrer" className="hover:text-emerald-400 transition-colors duration-200 flex items-center gap-2 group"><Phone className="h-4 w-4 group-hover:scale-110 transition-transform" />WhatsApp Support</a>
        </div>
      </div>
